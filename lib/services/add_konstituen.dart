...
class MarkingService extends GetxController {
  ...
  final errorMarking = ''.obs;
  final errorKonstituen = ''.obs;
  final markId = ''.obs;

  Future<bool> marking(int id, String nik) async {
    try {
      final token = await getTokenFromSharedPreferences();
      setAuthToken(token ?? '');
      final response = await _dio.post('${ApiService.markDptUrl}/$nik/mark',
          data: {'id_mark': id}, options: options);
  ...
  Future<bool> addDpt(int id, String? lat, String? long) async {
    try {
      final token = await getTokenFromSharedPreferences();
      setAuthToken(token ?? '');
      ...
      final response = await _dio.post(ApiService.markDptUrl,
          data: data,
  ...
  final agamaId = 1.obs;
  final difabelId = 1.obs;
  final profesiId = 1.obs;
  ...
  final RxList<Kabupaten> kabList = <Kabupaten>[].obs;
  final RxList<Kecamatan> kecList = <Kecamatan>[].obs;
